...
interface ApprovalLetterProps {
  ...
  /** Compatibilidad con el modal antiguo, que todavía envía formData. */
...
const ApprovalLetter = ({ professional, formData, documentDate }: ApprovalLetterProps) => {
  ...
  const documentDateValue = documentDate ||
    (p.fecha_generacion_resolucion
  ...
  return (
    <div
      ...
      className="bg-white"
      style={{ fontSize: "11px", lineHeight: "1.4", width: "210mm", minHeight: "297mm" }}
    >
      <div className="mb-4">
      ...
      <div className="text-right mb-4"><p>{p.distrito || "Malabo"}, {documentDateValue}</p></div>
      <div className="mb-3">
      ...
      <div className="mt-8 grid grid-cols-2 gap-8">
        <div className="text-center"><div className="border-t border-black w-48 mx-auto mb-2 mt-16" /><p className="font-semibold">DIRECTOR GENERAL</p><p className="text-sm">Recursos Humanos Sanitarios</p></div>
        <div className="text-center"><div className="border-t border-black w-48 mx-auto mb-2 mt-16" /><p className="font-semibold">MINISTRO DE SANIDAD</p><p className="font-semibold">Y BIENESTAR SOCIAL</p><p className="text-sm">República de Guinea Ecuatorial</p></div>
      </div>

      {(selloProfesional || selloExpediente) && (
        <div className="mt-6 flex items-start justify-center gap-12">
          {selloExpediente && <SelloDisplay sello={selloExpediente} titulo="Sello de expediente" compact size={100} />}
          {selloProfesional && <SelloDisplay sello={selloProfesional} titulo="Sello profesional" compact size={100} />}
        </div>
      )}
      ...
      <div className="mt-8 text-xs text-gray-600 text-center border-t pt-4">
        ...
        <p>Generado el {documentDateValue}</p>
      </div>
    </div>
  );
};
