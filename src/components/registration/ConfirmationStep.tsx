  ...
  useEffect(() => {
    ...
    const load = async (attempt = 0) => {
      ...
      try {
        ...
        let query = supabase.from("notas_ingreso").select("numero_nota,tipo_solicitud,concepto_descripcion,monto,moneda,cuenta_tesoreria,beneficiario_nombre,beneficiario_documento,hash,algoritmo,created_at").order("created_at", { ascending: false }).limit(1);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><FileText className="w-5 h-5 text-guinea-teal"/><span>Documentos individuales</span></CardTitle></CardHeader><CardContent><div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="border rounded-lg p-4"><h4 className="font-semibold mb-2">Nota de ingreso</h4><p className="text-sm text-gray-600 mb-3">{notaIngreso ? `Nota ${notaIngreso.numero_nota}` : notaLoading ? "Buscando la nota generada..." : "La nota aún no está disponible."}</p>{notaIngreso ? <Button variant="outline" asChild className="w-full"><a href={(notaIngreso as any).pdf_url} target="_blank" rel="noreferrer" download><ReceiptText className="w-4 h-4 mr-2"/>Descargar Nota de Ingreso</a></Button> : <Button variant="outline" disabled className="w-full"><ReceiptText className="w-4 h-4 mr-2"/>Esperando generación</Button>}</div>
