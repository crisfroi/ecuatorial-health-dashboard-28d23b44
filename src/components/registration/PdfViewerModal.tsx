...
const PdfViewerModal=({isOpen,onClose,formData,pdfType,notaIngreso}:Props)=>{
 const ref=useRef<HTMLDivElement>(null);const [pdfUrl,setPdfUrl]=useState<string|null>(null);const [loading,setLoading]=useState(true);const [blob,setBlob]=useState<Blob|null>(null);
 const generate=useCallback(async()=>{setLoading(true);if(pdfUrl)URL.revokeObjectURL(pdfUrl);setPdfUrl(null);setBlob(null);await new Promise(r=>setTimeout(r,250));const root=ref.current;if(!root){setLoading(false);return;}try{const targets=Array.from(root.children).filter((el):el is HTMLElement=>el instanceof HTMLElement);const pdf=new jsPDF('p','mm','a4');for(let i=0;i<targets.length;i++){const canvas=await html2canvas(targets[i],{scale:2,useCORS:true,allowTaint:false,backgroundColor:'#fff',imageTimeout:15000});if(i>0)pdf.addPage();pdf.addImage(canvas.toDataURL('image/png'),'PNG',0,0,210,297,undefined,'FAST');}const out=pdf.output('blob');setBlob(out);setPdfUrl(URL.createObjectURL(out));}catch(e){console.error('Error generando expediente PDF',e);setPdfUrl(null);}finally{setLoading(false);}},[formData,notaIngreso,pdfUrl]);
 useEffect(()=>{if(isOpen&&pdfType)generate();return()=>{if(pdfUrl)URL.revokeObjectURL(pdfUrl);};},[isOpen,pdfType,generate]);
