...
export const AuthProvider: React.FC<AuthProviderProps> = ({
  ...
  const getProfileFromDb = async (uid: string) => {
    try {
      ...
      if (error) {
        ...
        // Lanza un error para ser capturado por resolveRoleAndProfile o initializeAuth
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      try {
        ...
        if (AuthErrorHandler.isRefreshTokenError(error)) {
          ...
          // Esto forzará una redirección o limpieza de sesión completa.
          await AuthErrorHandler.handleRefreshTokenError(); 
          return; // Detener la ejecución para evitar setIsLoading(false) inmediato
        }
        
        // Si es cualquier otro error (ej. de red), limpiar la sesión y continuar.
        setUser(null);
        setUserRole(null);
      } finally {
        if (mounted) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        // En un refresh, el usuario sigue siendo el mismo. No hace falta re-resolver, solo actualizar el token.
        // Si el estado local se había perdido, podrías llamar a resolveRoleAndProfile, pero normalmente no es necesario.
  ...
  const login = async (email: string, password: string): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      // Fallback de contraseña (Si aún lo necesitas, sino, puedes eliminar estas líneas)
      ...
      // Esta línea es la que probablemente veías antes en un fallo de red
  ...
  const checkPermission = (permission: string): boolean => {
    ...
    // console.log(`🔐 Checking permission '${permission}' for role '${userRole}':`, hasPerms); // Comentar para reducir logs
    return hasPerms;
  };
  ...
  const checkTabAccess = (tab: string): boolean => {
    ...
    // console.log(`🔐 Checking tab access '${tab}' for role '${userRole}':`, canAccess); // Comentar para reducir logs
    return canAccess;
  };
