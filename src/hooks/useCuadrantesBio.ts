...
// useCuadrantesBio ya no llama a ningún hook.
...
    // Se ha implementado el filtro de profesionales con cuadrante asignado.
...
      // 1. OBTENER IDs de profesionales CON cuadrante asignado en el rango y centro
...
        .in('id', profIdsToExport); // CRÍTICO: Filtrar solo los que tienen cuadrante
...
      const profs = (data || []);
      
      // 3. CONSTRUIR el TSV (16 columnas)
      const headers = [
...
      const rows = profs
        .filter((p: any) => p.id_profesional_unico) // Solo si tienen EnNo asignado
        .map((p: any) => {
          const enNo = String(p.id_profesional_unico || '').replace(/^0+/, '').slice(0, 8); 
