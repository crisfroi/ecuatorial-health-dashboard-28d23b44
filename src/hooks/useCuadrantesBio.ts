...
    // Corregida la firma a 'triggerToast' para evitar TypeError y añadir manejo de errores.
...
    // La función exportCuadrantesXls debe estar sintácticamente correcta aquí:
...
      });
      const tsv = [headers, ...rows].map(r => r.join('\t')).join('\r\n');
      const blob = new Blob([tsv], { type: 'application/vnd.ms-excel' }); // <- Línea 141, debe estar completa.
      const a = document.createElement('a');
