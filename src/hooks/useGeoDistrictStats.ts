...
export const useGeoDistrictStats = () => {
    ...
    const data: DistritoGeoStats[] = useMemo(() => {
        ...
        return distritosQuery.data.map(d => {
            ...
            // -------------------------------------------------------------------
            // ⚠️ TODO: REEMPLAZAR SIMULACIÓN ⚠️
            // Aquí es donde en el futuro integrarás la lógica para obtener
            // las estadísticas reales por distrito. Por ahora, es una simulación.
            // Los nombres de las claves deben ser los nombres limpios:
            // cleanName = getCleanGeoName(nombre_distrito)
            // -------------------------------------------------------------------
            const simulatedStats: Record<string, { prof: number, cent: number }> = {
                'akonibe': { prof: 15, cent: 3 },
                'akurenam': { prof: 10, cent: 2 },
                'anisok': { prof: 18, cent: 3 }, 
                'baney': { prof: 12, cent: 2 },
                'bata': { prof: 60, cent: 8 },
                'cogo': { prof: 5, cent: 1 },
                'ebebiyin': { prof: 25, cent: 4 },
                'evinayong': { prof: 8, cent: 1 },
                'luba': { prof: 10, cent: 2 },
                'malabo': { prof: 90, cent: 12 },
                'mbini': { prof: 6, cent: 1 },
                'micomiseng': { prof: 22, cent: 4 },
                'mongomo': { prof: 16, cent: 3 },
                'niefang': { prof: 14, cent: 2 },
                'nsok nsomo': { prof: 7, cent: 1 },
                'nsork': { prof: 9, cent: 1 },
                'oyala': { prof: 5, cent: 1 }, 
                'riaba': { prof: 4, cent: 1 },
                'san antonio de pale': { prof: 3, cent: 1 },
            };
