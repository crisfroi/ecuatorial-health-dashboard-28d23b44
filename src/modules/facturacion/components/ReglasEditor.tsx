...
import { useRenaprosaReglas, type CreateReglaInput, type UpdateReglaInput } from '../hooks/useRenaprosaReglas';

interface ReglaFormState extends CreateReglaInput {
...
export default function ReglasEditor() {
  ...
  const handleSubmit = async () => {
    ...
    if (editingId) {
      actualizarRegla({ id: editingId, ...data } as UpdateReglaInput);
    } else {
      crearRegla(data as CreateReglaInput);
    }
      ...
      <Card>
        <CardContent className="pt-6">
          {isLoadingReglas ? (
            ...
            <div className="flex justify-center py-8">
              <p className="text-red-600">Error: {reglesError instanceof Error ? reglesError.message : 'Error desconocido'}</p>
            </div>
